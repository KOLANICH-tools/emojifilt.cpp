...
* Build and install https://github.com/KOLANICH-libs/libmo.cpp - a library for looking up values in `libintl`/`gettext` `mo` translation files. Since it seems `libintl`/`gettext` don't allow to do hashtable lookup using the concrete file, we use an own lib for it.
...
* Generate the database of mapping emoji slugs to code point sequences using [`emojiSlugMappingGen`](https://github.com/KOLANICH-tools/emojiSlugMappingGen.py) tool and save it to `/usr/share/emojifilt/emoji.mo` tool: `python3 -m emojiSlugMappingGen > emoji.mo && sudo fakeroot cp ./emoji.mo /usr/share/emojifilt/emoji.mo` .
